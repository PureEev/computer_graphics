...
void Dx11App::Render() {
    ...
    struct RenderObject { DirectX::XMVECTOR pos; DirectX::XMFLOAT4 color; bool isTransparent; float distToCam; };
    std::vector<RenderObject> objects = {
        { XMVectorSet(-3.0f, 0, 0, 1), {1,1,1,1}, false, 0 },
        { XMVectorSet(3.0f, 0, 0, 1), {1,1,1,1}, false, 0 },
        { XMVectorSet(0, 0, -2, 1), {1,1,1,0.5f}, true, 0 }
    };

    for (auto& obj : objects) {
        obj.distToCam = XMVectorGetX(XMVector3Length(obj.pos - m_cameraPos));
    }

    std::sort(objects.begin(), objects.end(), [](const RenderObject& a, const RenderObject& b) {
        if (a.isTransparent != b.isTransparent) return a.isTransparent < b.isTransparent;
        if (!a.isTransparent) return a.distToCam < b.distToCam;
        return a.distToCam > b.distToCam;
        });

    UINT strideCube = sizeof(Vertex), offset = 0;

    // --- 1. Отрисовка НЕПРОЗРАЧНЫХ кубов ---
